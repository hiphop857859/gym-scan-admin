...
const MenuStylePage = () => {
  ...
  const { openModalDelete, closeModalDelete } = useModals({
    handleModalDeleteOk: (value) => {
      value && deleteMusicStyle({ vars: { id: value?.toString() } })
    }
  })
  ...
  const { fetch: deleteMusicStyle } = useQuery({
    func: Service.deleteMusicStyle,
    onSuccess: () => {
      showSuccess('Delete music style success')
      closeModalDelete()
      handleOk()
    }
  })
