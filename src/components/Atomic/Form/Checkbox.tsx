...
const Checkbox: React.FC<CheckboxProps> = ({ label, checked, id, onChange, className = '', disabled = false }) => {
  return (
    <label className={`flex items-center space-x-3 cursor-pointer ${disabled ? 'cursor-not-allowed opacity-60' : ''}`}>
      <input
        id={id}
        type='checkbox'
        ...
        disabled={disabled}
      />
      {label && <span className='font-medium text-gray-800 text-theme-sm dark:text-white'>{label}</span>}
    </label>
  )
}
